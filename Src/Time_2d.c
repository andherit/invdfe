...
/*        The previous version was dated 11 March 1999                       */
/*        This "robustified and ANSIfied" version is dated 11 July 2003      */
/*                                                                           */
/* PROTOTYPE :                                                               */
/*                                                                           */
...
/*       equivalent in C (tab(i,j) in Fortran "means" tab[j-1][i-1] in C).   */
...
/*                                                                           */
/* Compiling:                                                                */
/*       If the compiler complains about infinity(), isinf(), nint(), or     */
/*       if the linker complains about undefined symbols _infinity _isinf    */
/*       and/or _nint, recompile using the preprocessor option               */
/*       -DNO_IEEE_PROTOCOL                                                  */
/*                                                                           */
...
/*                                   Note that no element of HS may be       */
/*                                   negative (unchecked): this would be     */
/*                                   physically absurd and would provoke a   */
/*                                   crash (infinitely recursive loop).      */
/*                                                                           */
...
/*                                                                           */
/*        What "robustification" and why ?                                   */
/*        I became recently aware of vicious problems occurring on some      */
/*        platforms with this program, due to the fact that its behaviour    */
/*        critically depends on "exact" comparisons between float values.    */
/*        The code behaves badly if a test of the form (a<b) appears         */
/*        true when a and b should be trusted to have exactly identical      */
/*        values, as they were computed with exactly the same instructions   */
/*        in two distinct parts of the source code. Unfortunately, such an   */
/*        error DOES pseudo-randomly occur on some platforms, in particular  */
/*        on Intel-based PCs under Linux. This may unpredictibly trigger     */
/*        an infinite loop of recursive calls (ending up with a crash !)     */
/*                                                                           */
/*        The origin of the problem as explained in gcc.info :               */
...
/*                                                                           */
/*        In this version, critical comparisons have been "fuzzified" i.e.,  */
/*        (a < b) was changed into (b - a > some_relevant_epsilon).          */
/*        Changes are indicated by a "FUZZIFIED COMPARISON" comment.         */
/*        Fuzzification is steered by the value of macro EPS_FUZZY.          */
/*        The value used (1.2e-07) is an approximation of the relative       */
/*        difference between nearest exactly representable floating point    */
/*        numbers in the IEEE754 encoding norm (using 23 mantissa bits).     */
/*        Results provided by this version will not be *exactly* the         */
/*        same as the ones provided by the "unfuzzified" code, but the       */
/*        differences are definitely benign considering the imprecision      */
/*        associated with the use of finite-differences.                     */
/*                                                                           */
/*                                                                           */
...
#define    ISINF(x)    ((x)>1.0e+19)
#define    ISNEGINF(x) ((x)<-1.0e+19)
#define    NINT(x)     (int)floor((x)+0.5)
/* NINT = "nearest integer", used here only with positive values... */
#else
#ifdef sun
#ifdef BSD
#include <sunmath.h>
...
static char    *err_msg[]=
            {
                ...
                "\ntime_2d: Multiple source with -Inf in input time map.\n",
            };
...
/* multiple source */
        for(x=0,min_t=max_t=t[0][0];x<nx;x++)
            for(y=0;y<ny;y++){
                if(t[x][y]<min_t){
                    min_t=t[x][y];
                    xs=x;
        ...
        if(ISNEGINF(min_t)) return ERR_MULTNEG;
        else if(ISINF(min_t) || max_t==min_t) return ERR_MULTUNI;
        source_at_node=1;
        X0=X1=xs;
        Y0=Y1=ys;
...
/* and finally undone at return time (see time_2d() and time_2d_()).     */
	if(init_stage==0){
	  if(min_t<0.0){
	    ...
	    for(x=0;x<nx;x++)
	      for(y=0;y<ny;y++)
	        if(!(ISINF(t[x][y]))) t[x][y]-=timeshift;
